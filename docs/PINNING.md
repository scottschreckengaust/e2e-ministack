...
| threat-composer-ai (uvx)                               | `.mcp.json` (canonical → generated per-agent files) | git commit SHA (one source, propagated by `scripts/sync-mcp-config.ts`; `npm run check:mcp-parity` gate) |
