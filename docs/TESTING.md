...
| Functional — infra                | unit tier — Jest + CDK `Template` (`test/unit/stack.test.ts`)                          | Fine-grained CDK assertions (security/structural intent) **plus** a fast-tier cdk-nag `AwsSolutions` assertion via `validateScope(stack)`; synth-only, no emulator | **Implemented**                                   |
...
- `npm run test:unit` — logic + CDK assertions + fast-tier cdk-nag assertion +
  fast-check properties (no emulator/Docker).
...
by cdk-nag/checkov plus the fine-grained CDK `Template` assertions instead.
