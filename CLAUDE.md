...
- **Dependency/supply-chain** — `npm audit --audit-level=high`, OSV-Scanner (lockfile), Grype (filesystem), `dependency-review-action` (PR-time dep-diff: vulns + a **deny-list license policy** blocking copyleft/AGPL/non-FOSS introductions, `fail-on-severity: high`), and Syft (CycloneDX SBOM artifact, informational). Full posture, the license-deny rationale (why deny-list not allow-list), and the intentional pre-commit↔CI gap live in **[docs/SECURITY-TOOLING.md](docs/SECURITY-TOOLING.md)** (the single source of truth).
