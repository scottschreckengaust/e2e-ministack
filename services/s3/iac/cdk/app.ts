...
import { CompatS3Stack } from './stack';

/**
 * The per-vertical CDK app entrypoint for the S3/CDK compat vertical (epic
 * #117, #139).
 *
 ...
 * `Contract`. Here the S3 vertical ships one stack.
 *
 * Mirrors `bin/app.ts` and the Lambda vertical's app.ts: the deploy target is
 ...
 *   cdk deploy CompatS3Stack --require-approval never \
 *     --app "npx ts-node --prefer-ts-exts services/s3/iac/cdk/app.ts"
...
export function buildCompatApp(): cdk.App {
  ...
  // Mirrors the idiom in services/lambda/iac/cdk/app.ts.
