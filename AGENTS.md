...
docker run -d --name ministack --network host \
  ...
  ministackorg/ministack:full@sha256:636c4ef52bff20e29f161d24e895359b2927f72a143d726792faa86160043ca9
...
- **Test strategy** — tiers (`jest.config.js` picks the tier dir via `JEST_TIER`): `test/unit/` (synth-only handler logic + CDK `Template` fine-grained assertions and a fast-tier cdk-nag `AwsSolutions` assertion via `validateScope`), **`integ/`** (**`@aws-cdk/integ-runner`** cloud-assembly snapshot baseline in `integ/*.js.snapshot/` — synth-only gate via `npm run test:integ-snapshot`; refresh with `npm run test:integ-snapshot:update` against running MiniStack, #168), `test/integration/` (AWS SDK v3 against deployed MiniStack resources, assumes `cdk deploy` ran), `test/e2e/` (`describe.skip` real-account placeholder). The full strategy — reframed as a parallel quality **matrix** (functional / performance / security / acceptance / mobile) with per-dimension status — lives in **[docs/TESTING.md](docs/TESTING.md)**, the single source of truth.
...
- **Mutation testing** — `npm run test:mutation` (Stryker) mutates `lambda/index.js` against the unit tier; CI gate breaks under 80% (currently 100%). Scoped to the Lambda logic only — the CDK stack is declarative config Stryker can't tie to synth output (mutants show as "no coverage"), so it's covered by cdk-nag/checkov/fine-grained assertions/integ-runner snapshots instead. `incremental: true` caches per-mutant verdicts; CI restores/saves that cache via `actions/cache`.
...
Two main workflows. `ci.yml` (jobs: changes → unit → integration) lints, runs unit tests + the cdk-nag synth gate + **`@aws-cdk/integ-runner` snapshot diff**, then deploys/tests against MiniStack. `security.yml` runs the scanners (also on a weekly cron). A small scheduled third, `license-review-poller.yml`, resolves open `license-review` issues against ClearlyDefined weekly — auto-closing on an allow-list-satisfiable declared license, escalating (and going red on an unacceptable one) otherwise (#127 Leg B; see docs/SECURITY-TOOLING.md).
...
- **checkov + cfn-lint** — scan synthesized `cdk.out` templates. checkov hard-fails (45 pass / 0 fail / 1 skip). `CKV_AWS_117` (Lambda-in-VPC) is skipped via CloudFormation `Metadata` (`checkov: { skip: [...] }`) injected with `cfnFn.addMetadata(...)` — the only way to suppress on CDK-generated templates. cfn-lint's expected `W3005` warning (redundant dependency) is not failed on.
- **Dependency/supply-chain** — `npm audit --audit-level=high`, OSV-Scanner (lockfile), Grype (filesystem [hard-fail] + the **hard-fail, VEX-gated** pinned-MiniStack-image scan), **Trivy** (Aqua Security, Apache-2.0 — a second vuln scanner added in #133: `trivy-fs` filesystem [report-only] + `trivy-image` pinned-image [**hard-fail, VEX-gated**] jobs mirroring the Grype pair, SARIF to the Security tab; a deliberate dual-scanner overlap since Grype and Trivy use different vuln DBs and surface different CVEs — on the pinned image the high+ sets diverge, so the `.vex/` set is their union; the count shifts on every digest/DB bump, see `.vex/README.md` for the current breakdown), `dependency-review-action` (PR-time dep-diff: vulns + an **allow-list license policy** — only the permissive licenses present in the tree pass, so copyleft/AGPL/non-FOSS introductions fail by omission; `fail-on-severity: high`), and Syft (CycloneDX SBOM artifact, informational). Full posture, the license-allow rationale (why allow-list not deny-list — `deny-licenses` is deprecated upstream), and the intentional pre-commit↔CI gap live in **[docs/SECURITY-TOOLING.md](docs/SECURITY-TOOLING.md)** (the single source of truth). The same license line governs **adopting new tools and GitHub Actions** (their licenses aren't packages in the lockfile, so the CI dependency gate never sees them): AGPL/copyleft is a near-dealbreaker and single-vendor lock-in is avoided — the stance that rejected k6 for load testing (#73) and removed Renovate (PR #54 → #80), and cleared Trivy (Apache-2.0) for #133. Check a candidate tool's license against this line before wiring it in.
- **MiniStack image scan — hard-fail, VEX-gated (#84)** — the `ministack-image` (Grype) and `trivy-image` (Trivy) jobs fail CI on any high+ CVE in the pinned emulator image **not** covered by an OpenVEX record under **`.vex/`**. The unfixable base-image CVEs (one record per accepted CVE — the set is `ls .vex/CVE-*.openvex.json`) are each accepted via an individual `.vex/CVE-XXXX.openvex.json` (`status: not_affected`, justification `vulnerable_code_cannot_be_controlled_by_adversary` — a genuine adversary-reachability claim for a local-only, loopback-bound, ephemeral CI emulator; accepted-risk prose + fix state in `impact_statement`). **Why `not_affected` not `affected`:** neither scanner suppresses `affected` (Grype's `AugmentMatches` re-surfaces it; both filter only `not_affected`/`fixed`) — proven in #160. This is an **honest, machine-readable, per-CVE risk acceptance** — NOT the forbidden blanket-ignore. Feed channel differs per scanner (neither `--vex` takes a directory): Grype via the `GRYPE_VEX_DOCUMENTS` env (comma-list, forwarded natively by `anchore/scan-action`); Trivy via the committed `trivy.yaml` `vulnerability.vex` list (auto-discovered — the `trivy-action` v0.36.0 forwards no `vex`/`TRIVY_VEX`, so the config file is the working channel). **Severity floor** starts at `high` and is a documented **ratchet** (high → medium → low, each step its own reviewable batch). `.vex/` staleness + the cutoff are #76 audit targets. Details: `.vex/README.md`, [docs/SECURITY-TOOLING.md](docs/SECURITY-TOOLING.md) § MiniStack image scan.
- **SAST/secrets** — Semgrep (`--config=auto --error`), Gitleaks (full history), CodeQL (JS/TS).
- **ClamAV** (#149) — signature-based virus/malware scan of the working tree (`clamav` job). External scanner (GPLv2, `clamav/clamav` Docker service + apt `clamdscan`) — the same external-invocation carve-out as shellcheck, so no copyleft dependency; **hard-fail**. Signatures float via freshclam (image pinned by digest); clamdscan's text log → SARIF via `.github/scripts/clamav-to-sarif.mjs` (every hit `error`/`security-severity` 10.0). Verify detection with EICAR on a throwaway path — **never commit it**, the job scans `.`.
- **SonarQube** (#150) — SonarQube Community code-quality + security analysis (`sonarqube` job), credential-free against a service container. SARIF is derived from `api/issues/search` by the in-repo `.github/scripts/sonar-to-sarif.mjs` — deliberately **not** `okorach/sonar-tools`, which hard-requires GPL-2.0 `levenshtein`. **Report-only** (mirrors `trivy-fs`), ratcheting to enforce later. **Governance exception:** the two SonarSource actions are LGPL-3.0 and SonarQube is single-vendor — a maintainer-approved deviation under the CI-only external-invocation carve-out; the LGPL scan-action is exempted in `dependency-review` via `allow-dependencies-licenses` (#161, verified name-only match). A proven false-positive of the `detected-sonarqube-docs-api-key` secrets rule on the digest/SHA pins is cleared with a **temporary `--exclude-rule`** on the semgrep job (NOT inline `# nosemgrep`, which leaves the finding in the SARIF for Code Scanning to surface); remove it once upstream semgrep/semgrep-rules#3994 lands (#163). See docs/SECURITY-TOOLING.md.
...
- Hooks: standard hygiene (large files, merge conflicts, shebang/executable consistency, EOF/whitespace, `check-json`/`check-yaml`, private-key detection), **gitleaks** (secrets — replaces the upstream `detect-aws-credentials` hook, which false-positives on `@aws-cdk/*` package names, `test/**` gitignore lines, and CDK snapshot metadata), **actionlint** (workflows), **shellcheck** (standalone `*.sh` scripts, via `shellcheck-py`), and `local` **eslint** + **tsc** + **markdownlint** + **prettier** that reuse the repo's pinned `node_modules` (so the hook and CI run identical tooling).
