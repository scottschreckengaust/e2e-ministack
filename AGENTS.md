...
- **shellcheck** — gates **standalone** `*.sh`/`*.bash` scripts (the `shellcheck` job in `ci.yml`, path-gated on shell changes; mirrored by a pre-commit hook). This is distinct from actionlint, which only shellchecks `run:` blocks _inside_ workflow YAML and never sees standalone scripts. shellcheck (GPLv3) is invoked as an external linter — not linked or redistributed — so it adds no copyleft dependency.
