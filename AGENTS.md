...
- **MCP / agent config** — the repo ships a Model Context Protocol config so an agent can drive the project and GitHub repo. `.mcp.json` is the Claude Code location; other agents read their own paths. See [docs/MCP.md](docs/MCP.md) for setup, the canonical-server-block convention, and the local-vs-remote rationale.
